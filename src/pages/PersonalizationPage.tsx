...
export default function PersonalizationPage() {
    ...
    const availableLanguages = book.languages || [];
    ...
    const addToCartMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            // Clear personalization state so next time creates a NEW one
            console.log('🔄 Clearing personalization state for next use');
            setPersonalization(null);
            setFormData(null);
            setCurrentStep('child-info');
    ...
    return (
        <PersonalizationWizard currentStep={currentStep} onStepChange={handleStepChange}>
            {currentStep === 'child-info' && (
                <ChildInfoForm
                    key={`child-info-${personalization?.id || 'temp'}-${formData ? 'has-data' : 'no-data'}`}
            ...
            {currentStep === 'preview' && (
                <div className="text-center py-12">
                    ...
                    <p className="text-gray-600 mb-6">
                        Review your personalization details below. Click "Add to Cart" when you're ready to proceed.
                    </p>
                    
                    {/* Personalization Summary - Use formData if personalization doesn't exist */}
                    {(personalization || formData) && (
                        <div className="bg-gray-50 rounded-lg p-6 mb-6 text-left max-w-md mx-auto">
                            <h3 className="font-semibold text-gray-900 mb-4">Personalization Summary</h3>
                            <div className="space-y-2 text-sm">
                                <div className="flex justify-between">
                                    <span className="text-gray-600">Child's Name:</span>
                                    <span className="font-medium text-gray-900">
                                        {personalization?.childFirstName || formData?.childFirstName || 'N/A'}
                                    </span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-gray-600">Age:</span>
                                    <span className="font-medium text-gray-900">
                                        {personalization?.childAge || formData?.childAge || 'N/A'} years
                                    </span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-gray-600">Language:</span>
                                    <span className="font-medium text-gray-900">
                                        {((personalization?.languageCode || formData?.languageCode || languageParam)).toUpperCase()}
                                    </span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-gray-600">Gender:</span>
                                    <span className="font-medium text-gray-900 capitalize">
                                        {personalization?.gender || formData?.gender || 'N/A'}
                                    </span>
                                </div>
                                {(personalization?.childPhotoUrl || formData?.childPhotoUrl) && (
                                    <div className="mt-4">
                    ...
                    {!personalization && !formData && (
                        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6 max-w-md mx-auto">
                            <p className="text-yellow-800 text-sm">
                                Please complete the personalization form first.
                            </p>
                        </div>
                    )}
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                        <button
                            ...
                            className="px-6 py-3 border-2 border-gray-300 rounded-xl hover:bg-gray-50 hover:border-gray-400 transition-all disabled:opacity-50 disabled:cursor-not-allowed font-semibold"
                        >
                            Back to Edit
                        </button>
                        <button
                            ...
                            className="px-8 py-4 bg-gradient-to-r from-indigo-600 to-blue-600 text-white rounded-xl hover:from-indigo-700 hover:to-blue-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed font-bold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 duration-200"
                        >
                            {isAddingToCart ? (
